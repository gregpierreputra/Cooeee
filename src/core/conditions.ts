...
/** When the feed was last read, or null when there is nothing honest to repeat:
 *  no snapshot yet, or one older than the same hour that hides a relief centre. */
const freshAsOf = (now: number, meta: NearbyCache['meta']): number | null => {
  const iso = meta.dynamic_source_last_success_at ?? meta.dynamic_generated_at ?? meta.dynamic_synced_at;
  if (iso === undefined) return null;
  const asOf = Date.parse(iso);
  return now - asOf > DYNAMIC_SNAPSHOT_MAX_AGE_MS ? null : asOf;
};

/** Whether a fresh heat notice names the point right now. */
export const heatNoticeAt = (now: number, cache: Pick<NearbyCache, 'conditions' | 'meta'>, point: LatLon): boolean =>
  freshAsOf(now, cache.meta) !== null && conditionsAt(point, cache.conditions).some((c) => c.hazard === 'heat');
 ...
 *  nothing honest to say. An empty `lines` means none apply here. */
export function noticeView(now: number, cache: Pick<NearbyCache, 'conditions' | 'meta'>, point: LatLon): NoticeView | null {
  const asOf = freshAsOf(now, cache.meta);
  if (asOf === null) return null;
  return {
    // One line per distinct notice: a publisher may issue the same title per district.
    lines: [...new Set(conditionsAt(point, cache.conditions).map((c) => copy.NOTICE_LINE(c.title, c.publisher)))],
